(function(){
  ...
  if (cueEl){
    cueEl.addEventListener('click', function(){
      var second = document.querySelectorAll('main .panel')[1];
  ...
  function flutePhrase(t0){
    ...
    for (var i = 0; i < motif.length; i++){
      ...
      g.gain.setTargetAtTime(0.055, t + 0.02, 0.15);
      t += dur;
    }
  ...
  function omVoice(detune, phaseMs){
    ...
    function cycle(){
      ...
      vg.gain.exponentialRampToValueAtTime(.7, t + 1.8);
      f1.frequency.linearRampToValueAtTime(300, t + 4.6);
      f2.frequency.linearRampToValueAtTime(430, t + 4.6);
      vg.gain.setValueAtTime(.7, t + 4.6);
      vg.gain.exponentialRampToValueAtTime(.35, t + 7.2);
  ...
  function startAudio(){
    ...
    clearTimers();
    playing = true;
    omVoice(0, 0); omVoice(.8, 4600);
    var notes = [98.0, 130.81, 130.81, 65.41], step = 0;
    function tanpura(){
      if (!ctx || !playing) return;
      pluck(notes[step % 4], ctx.currentTime + 0.03, 0.16);
      step++;
    }
