(function(){
  ...
  function voice(detune, phaseMs){
    ...
    var f1 = ctx.createBiquadFilter(); f1.type = 'bandpass'; f1.Q.value = 9;
    var f2 = ctx.createBiquadFilter(); f2.type = 'bandpass'; f2.Q.value = 10;
    var m1 = ctx.createGain(); m1.gain.value = .6;
    var m2 = ctx.createGain(); m2.gain.value = .35;
    ...
    function cycle(){
      ...
      f1.frequency.setValueAtTime(430, t);
      f2.frequency.setValueAtTime(880, t);
      vg.gain.setValueAtTime(0.0001, t);
      vg.gain.exponentialRampToValueAtTime(.5, t + 1.8);
      f1.frequency.linearRampToValueAtTime(215, t + 4.6);
      f2.frequency.linearRampToValueAtTime(330, t + 4.6);
      vg.gain.setValueAtTime(.5, t + 4.6);
      vg.gain.exponentialRampToValueAtTime(.26, t + 7.2);
  ...
  function build(){
    ...
    lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 1100;
    lp.connect(master);
    osc(54, .09); osc(108, .2); osc(108.7, .16); osc(216, .06); osc(324, .03);
  }
  ...
  function startAudio(){
    ...
    master.gain.setTargetAtTime(.13, ctx.currentTime, .9);
  }
  ...
  document.addEventListener('visibilitychange', function(){
    if (!ctx) return;
    if (document.hidden){ if (playing && ctx.suspend) ctx.suspend(); }
